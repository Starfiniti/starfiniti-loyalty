...
- M01-S01 advanced in production under ADR-0123. After an exact reviewed
  Compose check, fresh successful PostgreSQL archive, root-owned mode `0600`
  rollback artifact proven byte-equivalent after normalizing only the schema
  line, healthy eleven-service inventory, public smoke, and a
  Studio-only dry run, the stale Studio container was recreated with no image
  pull, build, or dependency start. It now carries the exact
  `public,graphql_public,loyalty` schema list on the same reviewed healthy image
  with zero restarts. The other ten Supabase services remained healthy and
  unrestarted; a post-change PostgreSQL archive passed; dashboard and login
  remained HTTP 200; unauthenticated Auth and Data API probes remained HTTP
  401; and commerce connections, customers, wallets, ledger transactions, and
  reward reservations remained zero. The 2026-08-28 evidence is preserved at
  exact SHA-256 and the V2 successor plus 52 corruption cases reject false
  scope, rollback, health, chronology, anonymous-access, or protected-state
  claims. Exact future Supabase upgrade rehearsal and complete clean-room
  recovery remain pending. Current branch is
...
- M09 adversarial review found a database-authority gap in customer-experience
  ...
  implementation `996d78227f310b949fe352264f655fde1f3bdb09` then passed CI
  `33525862937` and Security `33525862820` across all twelve checks, including
  all 70 pgTAP files and 3,845 assertions. The repository correction is
  complete; eligible independent PR review and the stacked merge chain remain
  pending. Production, M09's 88/100 score, checkout, coupons, ledger value, and
  tenant entitlements are unchanged.
...
- ADR-0119 preserves the accepted `starfiniti.recurring-failure-registry.v1`
  ...
  controls remain candidate-only until PR #58 and this stacked change receive
  independent review and merge. Fifty-eight adversarial cases pass. This changes
  no Release, deployment, production, score, or loyalty value state.
...
- ADR-0110 adds the still-missing exact Authentik 2026.8.0 runtime rehearsal
  ...
  the permissioned schedule `/send/` API. Reviewed artifact head
  `74a37e930cda44e4eedb550bd4a6237da03c75c5` passed CI `33384160196`,
  Security `33384160199`, and external CodeQL `99463328597`; all twelve PR
  ...
  WooCommerce evidence advances M15 security to 19/27. A new exact-artifact
  review reconciles all 29 Medium reciprocal-licence occurrences to 15 exact
  dispositions with zero false positives; 14 third-party dispositions remain
  release-blocking. The synthetic rehearsal cannot pass M13-S06's
...
- ADR-0094 extends recovery-byte preparation without touching production or rewriting accepted evidence. The V2 policy hash-binds the exact accepted thirty-entry BorgBackup/OpenSSH V1 policy and adds thirty-four rsync/governance entries: three candidate/dependency packages, all three rollback packages, the exact minimized canary report, forced exporter, host controller and compatible rollback scripts, four systemd units, sudoers rule, validators, canary inputs, runbook, provider and escrow ADRs, V1/V2 evidence, retained V1 policy, and exact verifier source. One shared no-network/no-copy/no-execution verifier now produces versioned manifests/reports and keeps package-authority, signing, dependency, host-consumer compatibility, offline custody, second review, isolated recovery, rollout, and operations escrow false. The complete 988-test local gate, static 87-migration/69-pgTAP validation, 1,154-file secret scan, zero-vulnerability production audit, and licence checks pass. Exact implementation `b2cbba0` passed CI `33244976784`, Security `33244976845`, and independent CodeQL `99080920204`; all twelve PR checks are green. IMP-010, R-004, M16 at 77/100, and production remain unchanged, and VM 971 was not accessed or modified.

- ADR-0093 adds the previously missing executable byte-verification boundary for BorgBackup/OpenSSH offline recovery escrow without touching production. A closed 30-entry policy covers exact candidate and rollback bytes, candidate executables, signing material, dependency inventories, build/verifier inputs, runbooks, ADRs, retained evidence, and canary reports. The cross-platform Node verifier has no network, copy, artifact-execution, installation, deletion, production-route, credential, or mutation path; it inventories only an operations-staged private directory, rejects missing/extra/case-colliding/linked/special/mutable/over-bound/digest-different/wrong-commit inputs through direct-parent, repeated stable no-follow descriptor reads and final closed-set checks, and writes only a private manifest plus an external minimized report. Adversarial review repaired parent-link, repeated-read/head-drift, provider-report leakage, duplicate-check, and post-publication path-mutation gaps. Exact implementation `504555c` passed CI `33243336082`, Security `33243336070`, and separate CodeQL check `99076519435`; all twelve checks are green. No real private inventory, offline copy, fingerprint/dependency review, second-person acceptance, recovery proof, or production authority exists, so both operations-escrow rows, R-004, M16 at 77/100, and production remain unchanged.

- ADR-0092 selects the next M16 recovery candidate without changing production. The privileged Proxmox backup client is directly exposed to a guest or recovery endpoint as its SSH server, so the OpenSSH 10.4 malicious-server client fix is relevant; the 10.5 agent, multiplexed remote-forward, and tunnel-restrict fixes are recorded without falsely claiming that those disabled features are active. The selected shape is only an upstream-signed 10.5p1 `ssh` below a versioned `/opt` root. The plan binds the exact Debian host client/rollback package, exact Ubuntu guest package line and `sshd` digest, archive/release checksum/signature/release-key fingerprint, 930-entry/892-file/10,059,047-byte safe source tree, client-only flags, strict effective options, internal no-port compatibility, bounds, exclusive output, and teardown. Bootstrap Security run `33240398639` discovered candidate executable SHA-256 `be9dd9ee2550e3fca2a6fa15edb5ab9303e42ac783ac766928fa5380971bf081`; exact candidate `275c9e8` then passed digest-locked Security run `33241151463` job `99070606112`. Its 741-byte retained report has SHA-256 `91b68dd8180324042e7dbea18ba26dc0e976cb4d977527845dc08f4689e6e276`, distinct from artifact archive SHA-256 `ddfe3ea8a2450d39251be96c38c9de69505eb88a6da5ce058f3d8151fc75f1fc`. Both vendor daemons, `/usr/bin/ssh`, keys, known hosts, backup consumers, checkout, and loyalty value remain untouched; escrow, real-provider proof, restore, rollout, rollback, monitoring, review, R-004, and M16 remain open.

- ADR-0091 closes one M16 candidate-selection ambiguity without changing production. Read-only installed evidence shows Debian Trixie `borgbackup=1.4.0-5`; Debian marks it affected by `CVE-2026-62268` with a no-DSA/minor disposition, while upstream Borg 1.4.5 contains the fix and Debian stable has no fixed package. The selected side-by-side candidate is Borg's locally built glibc 2.31 x86-64 single-directory release, not Debian unstable or a private backport. A closed plan binds the exact signed-Trixie rollback package and executable, candidate archive/signature/README/full primary fingerprint, executable, 106-entry/95-file/79,942,815-byte safe tree manifest, eight operation families, all four current/candidate client/server pairs, runtime ceilings, and false production authority. Exact implementation `fe727d5` passed Security run `33235799207`, job `99056449824`; the retained report and GitHub artifact archive have distinct verified SHA-256 bindings. The runtime proof is networkless, read-only, unprivileged, capability-free, bounded, and removed its exact container and image. Operations escrow, real-provider compatibility, every production consumer path, rollout, rollback, monitoring, isolated full-service restore, and independent review remain pending. M16 stays 77/100; production still uses 1.4.0-5 and no timer, repository, archive, checkout path, or loyalty value changed.

- ADR-0090 now turns ADR-0089's anonymous whole-host matrix into a bounded, still-unexecuted compatibility rehearsal for Critical R-059. The selected target is an isolated equivalent physical Proxmox host; nested-only evidence cannot close boot, KVM, IOMMU, or physical-network behavior. Synthetic guests cover all fifteen QEMU and four LXC profiles, while only the application and database use separately approved read-only recovery inputs bound to released Starfiniti `v0.1.11` and the exact reviewed Supabase `self-hosted/v0.8.0` compatibility, asymmetric-JWKS Compose, and `linux/amd64` image set. Thirteen canonical stages cover exact package bytes/install, candidate-kernel reboot, host/services/storage/profile behavior, critical Supabase/Auth/dashboard/worker/Authentik/WooCommerce/WAL/backup checks, zero-difference reconciliation, and controller teardown. A separate auto-destroy lease must equal the approval expiry so a controller crash or hard kill does not strand resources. The repository owns no endpoint, credential, SSH route, or environment driver. A real run requires an owner-bound clean commit, private target/control/driver inputs, a fresh minimized ADR-0088 dependency simulation whose file digest is approval-bound, and a fresh same-projection production inventory piped directly through the ADR-0089 validator; both observations must be within five minutes and only digests/timestamps survive. Thirty-nine adversarial cases pass, including stale/future/mismatched dependency or inventory facts, wrong critical releases/images, absent/late/unbound automatic destruction, routes, credentials, nested targets, impossible stage chronology, profile drift, wrong kernel, checkout dependency, reconciliation error, incomplete cleanup, false compatibility promotion, and immutable publication. Exact implementation head `741a375d67725dab0191d4f06bbd2779638c57b4` passed all seven CI jobs in run `33232583190`, all four Security jobs in run `33232583183`, and external CodeQL check `99048027493`; all twelve PR checks are green and PR #57 is merge-clean. No rehearsal, install, reboot, guest/storage/service change, or production mutation occurred; `rehearsalExecuted`, independent review, compatibility, rollback, recovery, repository policy, approvals, and post-change proof remain false. R-059 stays Critical.

- ADR-0089 now turns the next Proxmox compatibility prerequisite into a fail-closed whole-host contract without changing production. The exact projection covers 22 anonymous guests (18 QEMU and four LXC; 20 running and two stopped) across 19 behavior profiles, two storage profiles, nine required active management services, four host bridges, 71 guest-virtual interfaces, host boot/KVM/IOMMU facts, exact local tool digests, and zero HA resources. Both loyalty workloads are represented only by semantic aliases on one shared profile; VM IDs, names, raw configuration, storage IDs, interface names, addresses, MACs, paths, routes, credentials, and raw command output are prohibited. Fifty-six adversarial cases and exclusive publication pass. Exact implementation `e7825b6` produced the independently verified 9,236-byte artifact `proxmox-compatibility-inventory-e7825b6-2026-08-29T023345Z.json`, with file SHA-256 `f6af50f506044e7578dcd02f800c1c71680e322460bf81cf4faa705b0ff5e25f` and internal report SHA-256 `495d7960a59359794fdb5024171c2e2de66cf69fc7b6701447ae285b46ee376f`; no raw facts were retained. External CodeQL then rejected a High path-check/open race; correction `7526f55` uses no-follow descriptor/path identity before and after the read and passed CI `33229717718`, Security `33229717726`, and external CodeQL `99040438021` with all twelve checks green. Only consumer inventory capture advances. The six actual host/QEMU/LXC/storage/service/critical-workload rehearsals plus compatibility, recovery, rollback, repository policy, maintenance, install, reboot, mutation, and post-change proof remain false. R-059 stays Critical.

- ADR-0086 records a newly confirmed Critical production host gap without changing production: the running Proxmox kernel plus `pve-manager`, `qemu-server`, and `pve-container` are below fixed floors in five Proxmox security advisories published from 10–17 August 2026. A read-only signed-index APT simulation resolves an exact 165,341,024-byte dependency-complete candidate of eleven upgrades and one signed-kernel install with zero removals; its fixed metadata closes every listed advisory floor and retains the current rsync, BorgBackup, and OpenSSH recovery boundary. ADR-0087 binds a digest-pinned GitHub-hosted disposable proof of each fresh `InRelease` signature, signed uncompressed package index, APT exact-version URI, separately fetched exact URL, package fields, size, SHA-256, byte equality, zero installation, zero retention, and teardown. Nine networked attempts failed closed and remain recorded: keyring shape/mode/tool issues, an external CodeQL report-path race, Apt URI/hash parser assumptions, and cross-mount atomic publication. Head `45e9a12` then passed CI `33223681162`, Security `33223681183`, and external CodeQL `99023166148`; job `99022913369` produced artifact `9706126317`, which independently verifies five repositories, ten signatures, all twelve packages, and 165,341,024 bytes. Its exact committed 9,606-byte report has file SHA-256 `3eec19512a6b2535cf0c6359144c1807b78e01015f43c470ad53335c6eb1090e`, unchanged dpkg status, no installation or retained package bytes, no production credential/route/mutation, and successful teardown. ADR-0088 adds a distinct route-free production preflight: exact committed implementation artifact `proxmox-security-preflight-5659404-2026-08-29T013145Z.json` is 13,152 bytes with SHA-256 `b18037b19263020fabce46c2b6b13ec69b640775d2747dae474521191cba8a85` and proves the current installed starting state plus the exact twelve-action, zero-removal, zero-downgrade simulation with all bounded dpkg/APT/repository state unchanged. Head `8f2934b` passed CI `33226696854`, Security `33226696825`, and external CodeQL `99031930348`; all twelve PR checks are green. Package-byte, fresh signed-metadata, repository-signature, dependency-simulation, and installed-starting-state gates pass. Compatibility, recovery, rollback escrow, repository policy, maintenance, reboot, production mutation, and post-change reconciliation remain false or pending. R-059 and top-ranked IMP-011 stay Critical and require operations/security/owner approval, including an explicit enterprise-versus-no-subscription repository decision.

- ADR-0085 adds the next M16 evidence-preparation boundary without changing production. Read-only reconstruction covers Debian 13/trixie and Proxmox 9.2.3 on the host plus Ubuntu 24.04.4 on VM 971 with exact rsync, BorgBackup, OpenSSH, platform, and executable facts. The repository tool contains no SSH client or endpoint coordinates; it accepts only two bounded opaque fact envelopes, derives six provider projections, and binds exact historical committed official-source and full rsync-candidate-plan bytes. Thirty-six network-free/SSH-free adversarial cases pass, including canonical fact ordering, source-field smuggling rejection, repository-path containment, and candidate object/byte drift. The independently verified 8,813-byte artifact `recovery-dependency-snapshot-c5678b6-2026-08-28T221524Z.json` binds clean implementation `c5678b652024bb2a625f07d150e8ffd0b5d9e0cb` under SHA-256 `9960e01bea1a66856a2e0ed36493b28e183f5e316ce027fef3534ba5043448f7`. Installed capture is complete. ADR-0091 selects BorgBackup and its exact compatibility canary passes; ADR-0092 selects the OpenSSH client-only architecture and its digest-locked exact-plan compatibility canary now passes. Escrow, real-provider, activation, Debian, Ubuntu, Proxmox, broader human review, owners, schedules, approval, upgrade, and all production gates remain open.
...
- ADR-0081 established the fail-closed self-hosted Supabase compatibility boundary. ADR-0123 now closes its stale Studio runtime follow-up through an isolated recreation on the same exact image while preserving the prior baseline by digest. The current gate checks `self-hosted/v0.8.0` source/tag/commit, two reviewed Compose variants, fifteen mounted assets, eleven `linux/amd64` image IDs, Envoy/Auth/PostgREST/PostgreSQL/postgres-meta/JWKS behavior, 90 migrations, all 175 tenant tables, historical/current production evidence, and 52 deterministic corruptions. Exact future upgrade rehearsal and clean-room restore remain open.
...
- M15-S01 is active under ADR-0062 and ADR-0104. The repository-owned fixed-arrival driver still exercises readiness, authenticated customer account reads, scoped Service API intake, and exact-body signed WooCommerce intake. Its formerly weak independent-report boundary is replaced by exact Grafana k6 2.2.0 release/OCI-platform/script/workload provenance, canonical rational fixed-arrival schedules, descriptor-validated private authority/script snapshots, calling-UID/GID mounts, clean-commit recheck, contract-aware results, closed VU limits, zero dropped iterations, recomputed counts/error/latency decisions, minimized system tags, no cloud/usage/raw output, and explicit false production authority. Exact implementation commit `c8e3439` passed CI `33269532474`, Security `33269532376`, and CodeQL `99145597424`; all 12 PR checks were green, including the network-disabled digest-pinned Linux k6 image/script inspection. The complete local `npm run check` also passes with 995 tests, both production builds, all 23 capacity checks/corruptions, and every repository validator. Seven repository checks pass and 16 environment/inventory/data/monitoring/load/latency/reconciliation/repeatability/real-independent-run/claim checks remain pending. No supported capacity is claimed and no production load or mutation has run.
...
- M15-S03 is active under ADR-0064, ADR-0082, and ADR-0083. Reviewed artifact-head CI `33384160196` and Security `33384160199` at `74a37e9` are fully green: all seven baseline/database/container/WooCommerce jobs, all four Security jobs, CodeQL, repository scan, strict deployable-image scans, the worker runtime import probe, both CycloneDX inventories, exact reciprocal-source inventory, recovery transport, and isolated ZAP passed. The candidate retains the no-follow stable-descriptor correction for the real release-envelope race found by external CodeQL. The deployable response carries request-bound CSP/framing/MIME controls, and ZAP has zero Critical/High/Medium/Low alerts. The 27-check manifest records 19 passing checks; fresh digest-bound triage records zero false positives and R-056 blocks tagged images on 14 exact third-party source/notice obligations across 12 packages. Eight release/production/independent-test/final-reconciliation/approval gates remain non-passing. No production DAST, credential, customer data, provider call, tagged image, or loyalty-value mutation was authorized.
...
- M16 repository implementation is active under ADR-0068 and ADR-0084 through ADR-0093. Its canonical UTC contract requires two distinct consecutive monthly reviews, one five-exercise quarterly bundle, fourteen review sections, thirteen official provider/platform/recovery sources, exact backlog ranking, second-occurrence durable controls, material-change rescoring, guarded experiments, append-only ADR supersession, five digest-bound closeout artifacts, independent review, and five approvals. The clean-commit source collector's verified 6,534-byte artifact binds thirteen HTTP 200 source digests under SHA-256 `5786186426b065493f5c01b5d76742322e2e8ed3fe92b8f80c30d787caa516be`; the no-SSH installed-state collector's verified 8,813-byte artifact binds both opaque endpoints and six recovery-provider projections under SHA-256 `9960e01bea1a66856a2e0ed36493b28e183f5e316ce027fef3534ba5043448f7`. The verified Proxmox artifacts close fresh repository-signature, signed-index, exact package-byte provenance, dependency simulation, installed starting state, and consumer inventory only. ADR-0091 selects the exact Borg 1.4.5 candidate and its digest-bound four-pair Linux compatibility canary passes. ADR-0092 selects a client-only OpenSSH 10.5p1 architecture; exact candidate `275c9e8` passed the digest-locked Linux canary in Security run `33241151463` without production mutation. ADR-0093 adds a closed in-place byte verifier but no approved private inventory, offline custody, or independent review. Compatibility for the synthetic candidates now passes, but recovery, repository policy, approvals, execution, reboot, reconciliation, and every production activation gate remain open. The 2026-08-29 material-change rescore binds exact integrated candidate `cbe89b4` at an unchanged 83/100 and expands the exact ranked backlog from twelve to fourteen by separating whole-system fault exercise score 69 and capacity-envelope score 61; neither has approval or live evidence. Exact governance implementation `affa2ef` passed all twelve checks in CI `33270731237`, Security `33270731250`, and CodeQL `99148811211`; PR #57 is merge-clean. Seven repository controls pass and 32 elapsed-cadence, broader source/candidate review, exercise, reconciliation, review, and approval checks remain pending; the provisional score is 77/100 with performance and operability below their floors. No elapsed review, schedule, owner identity, production package upgrade, experiment, exercise, deployment, checkout path, or loyalty value changed.
...
- `npm run ci:validate` — passed locally for four baseline jobs, four security jobs, release SBOM/provenance, immutable action inputs, response-header checks, recovery transport, and container boundaries.
- `npm run security:validate` — passed its corruption matrix for 26 security checks, the digest-bound 30-to-16 Medium register, and the isolated six-job ZAP plan; 17 exact repository/execution controls pass and nine external/final-review gates remain deliberately non-passing.

...
const requiredChecks = new Set([
  "exact_head_ci",
  "contract_referral_matrix",
  "database_authority_matrix",
  "first_attribution_concurrency",
  "qualification_cooling_matrix",
  "give_get_ledger_matrix",
  "fraud_review_matrix",
  ...
  "non_canary_disabled",
  "advocate_link_canary",
  "first_attribution_canary",
  "attribution_window_canary",
  "self_referral_canary",
  "duplicate_identity_canary",
  "velocity_risk_canary",
  "paid_minimum_new_customer",
  "cooling_canary",
  "refund_before_issue",
  "advocate_reward_canary",
  "friend_reward_canary",
  "atomic_give_get_canary",
  "post_issue_refund_canary",
  "fraud_review_approval",
  "fraud_review_rejection",
  "exhausted_recovery_canary",
  "customer_share_progress",
  "merchant_funnel_fraud",
  "cross_tenant_denial",
  "attribution_reconciliation",
  "ledger_reconciliation",
  "fraud_evidence_reconciliation",
  "queue_continuity_reconciliation",
  "rollback_rehearsal",
  "observation_window",
...
const requiredAutomaticFails = new Map([
  [
    "identity_or_channel_leak",
    "an advocate code share URL projection log or artifact exposes customer channel network payment or address identity",
  ],
  [
    "self_referral_value",
    "a self-referral reaches attribution cooling qualification reward issuance or another value-bearing state",
  ],
  [
    "multiple_advocate_attribution",
    "one referred customer or canonical order becomes attributed to more than one advocate or programme policy",
  ],
  [
    "browser_authority",
    "browser input Auth claims email domain share parameters or connector metadata grants tenant customer referral or value authority",
  ],
  [
    "early_referral_value",
    "advocate or friend value issues before paid minimum new-customer cooling and review requirements all pass",
  ],
  [
    "duplicate_referral_effect",
    "duplicate replay delayed delivery or concurrent processing creates another attribution transition reward or compensation effect",
  ],
  [
    "partial_give_get",
    "advocate and friend rewards or their refund compensation commit partially or reconcile to different canonical referral evidence",
  ],
  [
    "refund_retains_value",
    "a qualifying refund fails to cancel value-neutral work or compensate each issued referral side exactly once",
  ],
  [
    "fraud_review_bypass",
    "a held or uncertain referral issues value without a live authorized reason-bound immutable review decision",
  ],
  [
    "unbounded_recovery",
    "fraud or reward recovery resets cumulative attempt identity exceeds its hard ceiling or hides terminal manual review",
  ],
  [
    "fraud_data_overretention",
    "raw identity network payment shipping evidence or purpose-separated fingerprints survive beyond the bounded private retention policy",
  ],
  [
    "cross_tenant_exposure",
    "an unrelated tenant programme customer attribution review job ledger effect fingerprint or evidence becomes visible or mutable",
  ],
  [
    "checkout_dependency",
    "WooCommerce checkout synchronously depends on the Hub worker entitlement attribution fraud review or referral reward outcome",
  ],
  [
    "accepted_work_stranded",
    "disabling referrals hides or strands an accepted attribution cooling review reward job or required compensation",
  ],
  [
    "sensitive_evidence",
    "reusable signing material raw payload personal data fingerprint values or private ledger metadata enters logs support output or evidence",
  ],
  [
    "unexplained_or_unapproved_close",
    "any attribution review job reward ledger funnel fraud tenancy or privacy difference approval artifact score floor or critical finding remains unresolved",
  ],
]);
const checkArtifactBindings = new Map([
  ...
  ["advocate_link_canary", ["canary_journal"]],
  ["first_attribution_canary", ["canary_journal"]],
  ["attribution_window_canary", ["canary_journal"]],
  ["self_referral_canary", ["canary_journal"]],
  ["duplicate_identity_canary", ["canary_journal"]],
  ["velocity_risk_canary", ["canary_journal"]],
  ["paid_minimum_new_customer", ["canary_journal"]],
  ["cooling_canary", ["canary_journal"]],
  ["refund_before_issue", ["canary_journal"]],
  ["advocate_reward_canary", ["canary_journal"]],
  ["friend_reward_canary", ["canary_journal"]],
  ["atomic_give_get_canary", ["canary_journal"]],
  ["post_issue_refund_canary", ["canary_journal"]],
  ["fraud_review_approval", ["canary_journal"]],
  ["fraud_review_rejection", ["canary_journal"]],
  ["exhausted_recovery_canary", ["canary_journal"]],
  ["customer_share_progress", ["canary_journal"]],
  ["merchant_funnel_fraud", ["canary_journal"]],
  ["cross_tenant_denial", ["canary_journal"]],
  ["attribution_reconciliation", ["reconciliation_report"]],
  ["ledger_reconciliation", ["reconciliation_report"]],
  ["fraud_evidence_reconciliation", ["reconciliation_report"]],
  [
    "queue_continuity_reconciliation",
    ["reconciliation_report", "rollback_report"],
  ],
...
const fail = (message) => {
  throw new Error(`Referral canary evidence invalid: ${message}`);
};
...
const safeArtifactPath = (relativePath, artifactId) => {
  ...
  const pattern = new RegExp(
    `^docs/plan/evidence/M06/production/referral-${artifactStem}-[a-z0-9][a-z0-9-]{2,79}\\.json$`,
    "u",
  );
  ...
  const allowed = `${resolve(root, "docs/plan/evidence/M06/production")}${sep}`;
...
const policyDigestKeys = [
  "pilotScopeSha256",
  "rolloutPolicySha256",
  "referralValueCeilingPolicySha256",
  "attributionPolicySha256",
  "coolingPolicySha256",
  "fraudReviewPolicySha256",
  "riskRetentionPolicySha256",
  "observationPolicySha256",
];

const referralEvidenceDigestKeys = [
  "advocateLinkSha256",
  "firstAttributionSha256",
  "attributionWindowSha256",
  "selfReferralSha256",
  "duplicateIdentitySha256",
  "velocityRiskSha256",
  "paidMinimumNewCustomerSha256",
  "coolingSha256",
  "refundBeforeIssueSha256",
  "advocateRewardSha256",
  "friendRewardSha256",
  "atomicGiveGetSha256",
  "postIssueRefundSha256",
  "fraudApprovalSha256",
  "fraudRejectionSha256",
  "exhaustedRecoverySha256",
  "customerShareProgressSha256",
  "merchantFunnelFraudSha256",
...
const referralCountKeys = [
  "advocateLinkCount",
  "firstAttributionWinnerCount",
  "competingAdvocateRejectedCount",
  "attributionWindowBoundaryCount",
  "selfReferralRejectedCount",
  "duplicateIdentityReviewCount",
  "velocityReviewCount",
  "newCustomerQualifiedCount",
  "coolingReleaseCount",
  "refundBeforeIssueCancellationCount",
  "advocateRewardEffectCount",
  "friendRewardEffectCount",
  "atomicGiveGetPairCount",
  "postIssueCompensationCount",
  "fraudApprovalCount",
  "fraudRejectionCount",
  "exhaustedRecoveryReviewCount",
  "customerExperienceCaseCount",
  "merchantExperienceCaseCount",
  "crossTenantDenialCount",
  "acceptedWorkCount",
  "riskFingerprintCount",
];
...
const validateArtifactDetails = (artifactId, document, candidateEvidence) => {
  ...
  if (artifactId === "release_inventory") {
    exactKeys(
      ...
      [
        ...
        "referralWorkerEnabled",
        "referralExperienceEnabled",
    ...
    if (
      ...
      details.referralFeatureEnabled !== false ||
      details.referralWorkerEnabled !== false ||
      details.referralExperienceEnabled !== false ||
  ...
  if (artifactId === "approval_record") {
    exactKeys(
      ...
      [
        ...
        "release",
        ...policyDigestKeys,
        "approvedMaxReferredCustomers",
        "approvedMaxRewardEffects",
        "approvedMaxPointsIssued",
        "approvedCoolingHours",
        "approvedRiskRetentionDays",
        "approvals",
        "artifactSha256",
      ],
    ...
    if (
      details.approvedMaxReferredCustomers > 10_000 ||
      details.approvedMaxRewardEffects > 100_000 ||
      details.approvedMaxPointsIssued > 1_000_000_000 ||
      details.approvedCoolingHours > 2_160 ||
      details.approvedRiskRetentionDays > 365
    ) {
      fail("approval_record referral ceiling differs");
    }
  ...
  if (artifactId === "production_baseline") {
    const digestKeys = [
      ...
      "referralPolicyTotalsSha256",
      "attributionTotalsSha256",
      "qualificationTotalsSha256",
      "reviewTotalsSha256",
      "rewardJobTotalsSha256",
      "ledgerTotalsSha256",
      "lotTotalsSha256",
      "reservationTotalsSha256",
      "nativeCommandTotalsSha256",
      "fingerprintTotalsSha256",
    ...
    exactKeys(
      ...
      [
        ...
        "sourceCoverageRatio",
        ...digestKeys,
        "activeReferralFeatureCount",
        "activeReferralWorkerCount",
        "activeReferralExperienceCount",
    ...
    if (
      ...
      details.activeReferralFeatureCount !== 0 ||
      details.activeReferralWorkerCount !== 0 ||
      details.activeReferralExperienceCount !== 0 ||
  ...
  if (artifactId === "canary_journal") {
    const zeroKeys = [
      "identityLeakCount",
      "selfReferralValueCount",
      "multipleAdvocateAttributionCount",
      "browserAuthorityCount",
      "earlyReferralValueCount",
      "duplicateReferralEffectCount",
      "partialGiveGetCount",
      "refundCompensationGapCount",
      "fraudReviewBypassCount",
      "unboundedRecoveryCount",
      "riskEvidenceOverretentionCount",
    ...
    exactKeys(
      ...
      [
        ...
        "approvedRiskRetentionDays",
        ...referralCountKeys,
        "observedReferredCustomerCount",
        "observedRewardEffectCount",
        "observedPointsIssued",
        "maximumCoolingHours",
        "maximumRiskRetentionDays",
        ...zeroKeys,
        "sourceCoverageRatio",
        "assertions",
      ],
    ...
    const exactScenarioCounts = {
      advocateLinkCount: 1,
      firstAttributionWinnerCount: 1,
      competingAdvocateRejectedCount: 1,
      attributionWindowBoundaryCount: 2,
      selfReferralRejectedCount: 1,
      duplicateIdentityReviewCount: 1,
      velocityReviewCount: 1,
      newCustomerQualifiedCount: 1,
      coolingReleaseCount: 1,
      refundBeforeIssueCancellationCount: 1,
      advocateRewardEffectCount: 1,
      friendRewardEffectCount: 1,
      atomicGiveGetPairCount: 1,
      postIssueCompensationCount: 2,
      fraudApprovalCount: 1,
      fraudRejectionCount: 1,
      exhaustedRecoveryReviewCount: 1,
      customerExperienceCaseCount: 1,
      merchantExperienceCaseCount: 1,
      crossTenantDenialCount: 1,
      acceptedWorkCount: 5,
      riskFingerprintCount: 3,
    };
    ...
    if (
      ...
      details.approvedCoolingHours > 2_160 ||
      details.approvedRiskRetentionDays > 365 ||
      details.observedReferredCustomerCount >
        details.approvedMaxReferredCustomers ||
      details.observedRewardEffectCount > details.approvedMaxRewardEffects ||
      details.observedPointsIssued > details.approvedMaxPointsIssued ||
      details.maximumCoolingHours > details.approvedCoolingHours ||
      details.maximumRiskRetentionDays > details.approvedRiskRetentionDays ||
      details.sourceCoverageRatio !== 1
    ) {
  ...
  if (artifactId === "reconciliation_report") {
    ...
    for (const key of referralCountKeys) {
    ...
    uniqueNonzeroDigests(
      details,
      [...referralEvidenceDigestKeys, ...truthDigestKeys],
      "reconciliation_report",
    );
    exactAssertions(
      details.assertions,
  ...
  if (artifactId === "rollback_report") {
    exactKeys(
      ...
      [
        ...
        "endedAt",
        "durationSeconds",
        "referralFeatureDisabled",
        "referralWorkerStopped",
        "referralExperienceDisabled",
        "acceptedWorkDrainedOrHeld",
        "priorImagesRestored",
        "priorPluginRestored",
        "policiesPreserved",
        "attributionsPreserved",
        "qualificationsPreserved",
        "reviewsPreserved",
        "rewardJobsPreserved",
        "ledgerPreserved",
        "fingerprintRetentionPreserved",
        "compensationsPreserved",
    ...
    if (
      ...
      requiredTrue.some((key) => details[key] !== true) ||
      requiredZero.some((key) => details[key] !== 0)
    ) {
  ...
  if (artifactId === "observation_report") {
    const zeroKeys = [
      ...
      "crossTenantExposureCount",
      "checkoutBlockedCount",
      "attributionDifferenceCount",
      "reviewDifferenceCount",
      "ledgerDifference",
    ...
    exactKeys(
      ...
      [
        ...
        "attributionP95LatencyMs",
        "qualificationP95LatencyMs",
        "reviewP95LatencyMs",
        "rewardP95LatencyMs",
        "customerExperienceP95LatencyMs",
    ...
    if (
      ...
      details.attributionP95LatencyMs > 60_000 ||
      details.qualificationP95LatencyMs > 60_000 ||
      details.reviewP95LatencyMs > 60_000 ||
      details.rewardP95LatencyMs > 60_000 ||
      details.customerExperienceP95LatencyMs > 60_000 ||
...
const validateDocument = (
  ...
  for (const artifact of candidateEvidence.artifacts) {
    ...
    if (
      document?.schema !== "starfiniti.referral-canary-artifact.v1" ||
  ...
  const m06 = candidateTasks.tasks.find((task) => task.id === "M06-REFERRALS");
  const requiredCompletedSlices = new Set([
    "M06-S01-POLICY-AND-FIRST-ATTRIBUTION",
    "M06-S02-QUALIFICATION-AND-COOLING",
    "M06-S03-GIVE-GET-LEDGER",
    "M06-S04-FRAUD-REVIEW",
    "M06-S05-CUSTOMER-AND-MERCHANT-EXPERIENCE",
  ]);
  const s06 = m06?.slices?.find(
    (slice) => slice.id === "M06-S06-CANARY-AND-CLOSE",
  );
  if (!m06 || !s06) fail("M06 or M06-S06 task is missing");
  for (const id of requiredCompletedSlices) {
    const slice = m06.slices.find((candidate) => candidate.id === id);
    if (slice?.status !== "completed") {
      fail(`${id} must be completed before canary`);
    }
  }
  if (m06.module_score !== calculatedScore) {
  ...
  if (candidateEvidence.status === "complete") {
    ...
    for (const key of policyDigestKeys) {
      if (approvalRecord.details[key] !== journal.details[key]) {
        fail(`approved referral ${key} differs from the canary journal`);
      }
    }
    for (const key of [
      "approvedMaxReferredCustomers",
    ...
    for (const key of releaseDigestKeys) {
      if (releaseInventory.details[key] !== journal.details[key]) {
        fail(`released referral ${key} differs from the canary journal`);
      }
    }
    for (const key of referralEvidenceDigestKeys) {
    ...
    for (const key of referralCountKeys) {
      if (
        ...
        fail(`referral artifact ${key} differs`);
      }
    }
    if (
      ...
      fail("referral observation policy differs from the journal");
    }
    ...
    if (m06.status !== "complete" || s06.status !== "completed") {
      fail("complete evidence requires completed M06 and S06 task state");
    }
  } else if (m06.status !== "in_progress" || s06.status !== "in_progress") {
    fail("in-progress evidence must match in-progress M06 and S06 task state");
  }
...
if (process.argv.includes("--self-test")) {
  ...
  const buildCompleteFixture = () => {
    ...
    const m06 = candidateTasks.tasks.find(
      (task) => task.id === "M06-REFERRALS",
    );
    m06.status = "complete";
    m06.module_score = candidateEvidence.score.total;
    m06.slices.find((slice) => slice.id === "M06-S06-CANARY-AND-CLOSE").status =
      "completed";
    ...
    const referralEvidenceDigests = fixtureDigests(
      referralEvidenceDigestKeys,
      "referral-evidence",
    );
    ...
    const fixtureDetails = (artifactId) => {
      ...
      if (artifactId === "release_inventory") {
        return {
          ...
          referralWorkerEnabled: false,
          referralExperienceEnabled: false,
      ...
      if (artifactId === "production_baseline") {
        ...
        return {
          ...
          activeReferralFeatureCount: 0,
          activeReferralWorkerCount: 0,
          activeReferralExperienceCount: 0,
      ...
      if (artifactId === "canary_journal") {
        return {
          ...
          ...policyDigests,
          ...releaseDigests,
          ...referralEvidenceDigests,
          approvedMaxReferredCustomers: 100,
          approvedMaxRewardEffects: 100,
          approvedMaxPointsIssued: 10_000,
          approvedCoolingHours: 720,
          approvedRiskRetentionDays: 90,
          ...referralCounts,
          observedReferredCustomerCount: 2,
          observedRewardEffectCount: 4,
          observedPointsIssued: 400,
          maximumCoolingHours: 720,
          maximumRiskRetentionDays: 90,
          identityLeakCount: 0,
          selfReferralValueCount: 0,
          multipleAdvocateAttributionCount: 0,
          browserAuthorityCount: 0,
          earlyReferralValueCount: 0,
          duplicateReferralEffectCount: 0,
          partialGiveGetCount: 0,
          refundCompensationGapCount: 0,
          fraudReviewBypassCount: 0,
          unboundedRecoveryCount: 0,
          riskEvidenceOverretentionCount: 0,
      ...
      if (artifactId === "reconciliation_report") {
        ...
        return {
          ...
          ...referralCounts,
          policyDifference: 0,
          attributionDifference: 0,
          qualificationDifference: 0,
          reviewDifference: 0,
          rewardJobDifference: 0,
          ledgerDifference: 0,
          lotDifference: 0,
          reservationDifference: 0,
          nativeRewardDifference: 0,
          fingerprintDifference: 0,
          queueDifference: 0,
          refundDifference: 0,
      ...
      if (artifactId === "rollback_report") {
        return {
          ...
          durationSeconds: 60,
          referralFeatureDisabled: true,
          referralWorkerStopped: true,
          referralExperienceDisabled: true,
          acceptedWorkDrainedOrHeld: true,
          priorImagesRestored: true,
          priorPluginRestored: true,
          policiesPreserved: true,
          attributionsPreserved: true,
          qualificationsPreserved: true,
          reviewsPreserved: true,
          rewardJobsPreserved: true,
          ledgerPreserved: true,
          fingerprintRetentionPreserved: true,
          compensationsPreserved: true,
      ...
      if (artifactId === "observation_report") {
        return {
          ...
          attributionP95LatencyMs: 250,
          qualificationP95LatencyMs: 300,
          reviewP95LatencyMs: 350,
          rewardP95LatencyMs: 400,
          customerExperienceP95LatencyMs: 200,
          ...
          crossTenantExposureCount: 0,
          checkoutBlockedCount: 0,
          attributionDifferenceCount: 0,
          reviewDifferenceCount: 0,
          ledgerDifference: 0,
    ...
    for (const artifact of candidateEvidence.artifacts.filter(
      ...
      const document = {
        schema: "starfiniti.referral-canary-artifact.v1",
      ...
      artifact.path = `docs/plan/evidence/M06/production/referral-${artifact.id.replaceAll("_", "-")}-self-test.json`;
    ...
    approvalArtifact.path =
      "docs/plan/evidence/M06/production/referral-approval-record-self-test.json";
  ...
  pendingFixture.candidateEvidence.checks.find(
    (check) => check.id === "first_attribution_canary",
  ).status = "pending";
  expectRejected(
  ...
  unsafeArtifactPath.artifacts.find(
    (artifact) => artifact.id === "read_only_baseline",
  ).path = "docs/plan/evidence/M06/canary.yaml";
  expectRejected(
    unsafeArtifactPath,
  ...
  expectRejected(
    ...
    "approved referral referralValueCeilingPolicySha256 differs",
    ...
    artifactMutationReader(
      ...
      "canary_journal",
      (document) => {
        document.details.referralValueCeilingPolicySha256 = digest(
          "fixture:different-referral-value-policy",
        );
      },
    ),
  );
  ...
  expectRejected(
    ...
    "approved referral approvedMaxPointsIssued differs",
    "a canary journal with a different points ceiling",
    approvedCeilingFixture.candidateTasks,
    artifactMutationReader(
      approvedCeilingFixture,
      "canary_journal",
      (document) => {
        document.details.approvedMaxPointsIssued = 9_999;
      },
    ),
  );
  ...
  expectRejected(
    ...
    "released referral wordpressPluginSha256 differs",
  ...
  const attributionEvidenceFixture = buildCompleteFixture();
  expectRejected(
    attributionEvidenceFixture.candidateEvidence,
    "canary and reconciliation firstAttributionSha256 differ",
    "a reconciliation report for different attribution evidence",
    attributionEvidenceFixture.candidateTasks,
    artifactMutationReader(
      attributionEvidenceFixture,
      "reconciliation_report",
      (document) => {
        document.details.firstAttributionSha256 = digest(
          "fixture:different-first-attribution",
        );
      },
    ),
  );
  ...
  expectRejected(
    ...
    "canary and reconciliation postIssueRefundSha256 differ",
    ...
    artifactMutationReader(
      ...
      "reconciliation_report",
      (document) => {
        document.details.postIssueRefundSha256 = digest(
          "fixture:different-post-issue-refund",
        );
      },
    ),
  );
  ...
  expectRejected(
    ...
    "referral artifact acceptedWorkCount differs",
    "a reconciliation report for a different accepted-work population",
    countMismatchFixture.candidateTasks,
    artifactMutationReader(
      countMismatchFixture,
      "reconciliation_report",
      (document) => {
        document.details.acceptedWorkCount = 6;
      },
    ),
  );
  ...
  expectRejected(
    ...
    "referral observation policy differs from the journal",
  ...
  const selfReferralValueFixture = buildCompleteFixture();
  expectRejected(
    selfReferralValueFixture.candidateEvidence,
    "canary_journal safety evidence differs",
    "a canary that issued self-referral value",
    selfReferralValueFixture.candidateTasks,
    artifactMutationReader(
      selfReferralValueFixture,
      "canary_journal",
      (document) => {
        document.details.selfReferralValueCount = 1;
      },
    ),
  );
  ...
  incompleteSliceTasks.tasks
    .find((task) => task.id === "M06-REFERRALS")
    .slices.find((slice) => slice.id === "M06-S04-FRAUD-REVIEW").status =
    "in_progress";
  expectRejected(
    evidence,

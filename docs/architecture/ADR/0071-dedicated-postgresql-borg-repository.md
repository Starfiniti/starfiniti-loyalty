...
- Amended: one explicit `archive|maintain` controller owns the active privileged boundary; the former two programs remain rollback artifacts
...
One installed `starfiniti-loyalty-postgres-borg-controller` accepts exactly one
`archive` or `maintain` subcommand. It validates and sources the root-equivalent
configuration once, and shares numeric state, atomic repository metrics, and the
actual repository-identity boundary. The two earlier standalone programs remain
unchanged rollback artifacts and are not referenced by the reviewed systemd units.
This reduces duplicated privileged configuration/metric logic without introducing a
sourced root-equivalent library or changing either mode's command allowlist.

Archive mode completes the restricted incremental rsync before acquiring the dedicated repository lock. Staging therefore remains fresh during a bounded retention operation; only borg create is serialized. Contention waits at most 120 seconds and exits 75 without claiming an archive.
...
5. Disable the legacy PostgreSQL prune timer, install the reviewed controller plus its isolated/conflicting archive and maintenance systemd units, retain the two former programs only as rollback artifacts, configure node-exporter's textfile collector for the reviewed directory, then run systemd-analyze verify.
...
Repository tests execute both explicit controller subcommands with mock rsync/Borg commands to prove successful dedicated routing, invalid-mode refusal, one shared privileged configuration/metric implementation, missing configuration, selector reuse and canonical-ID reuse refusal, staging-before-lock order, contention status, bounded check/prune/list/compact order, atomic minimized metrics, a visible over-bound retained interval, legacy-timer conflict, and no whole-VM lock acquisition. No production repository, script, timer, lock, archive, credential, monitoring service, or value changed when this ADR was accepted or amended.

## Rollback

Disable both dedicated-repository timers and preserve their repository, the previous shared-repository archives, the retained standalone controller artifacts, and the complete local base/WAL chain. A supervised rollback may point each disabled unit back to its matching retained standalone program only after syntax/config/mock verification; never run both generations concurrently. Restore the prior application package only for a supervised recovery action while archive freshness is explicitly failing. Never reintroduce silent success, bypass Borg locking, delete the dedicated repository, or prune the old PostgreSQL archive series until a replacement path has passed create and isolated restore evidence.

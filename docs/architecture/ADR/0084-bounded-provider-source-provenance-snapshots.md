...
7. Preserve committed provider-source snapshot bytes with a path-scoped Git `-text` attribute. The recorded artifact SHA-256 identifies the same bytes on Windows and POSIX checkouts rather than a platform-normalized rendering.
8. Keep deterministic self-tests network-free. They must reject catalogue and source drift, false review assertions, private or mixed DNS, unsafe or query-bearing redirects, invalid type or encoding, oversized, truncated, or incomplete bodies, invalid digests, relative output, a missing output parent, and output reuse.

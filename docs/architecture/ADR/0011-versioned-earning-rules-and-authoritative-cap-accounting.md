...
Live activity processing will acquire a transaction-scoped organization/customer advisory lock before reading immutable per-rule usage, evaluate with the same pure function used by simulation, append evaluation and usage evidence, and post the ledger award in one transaction. Cap windows use UTC until the campaign/timezone module introduces an explicit programme timezone contract. A new rule code starts new member-cap identity; keeping a code across programme versions preserves accumulated usage.

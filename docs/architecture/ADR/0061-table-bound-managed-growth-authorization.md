...
- The trigger delegates to `evaluate_managed_growth_boundary_v1`, which calls the separate ADR-0060 growth authorization using a stable non-personal subject. Commercial enforcement activates only when the managed tenant's explicit `managed.billing` canary entitlement is enabled; while that server-side flag is disabled, the ordinary feature-entitlement result remains authoritative. A missing boundary, organization, operation, entitlement, or enabled commercial allowance fails closed.
- Authenticated browser requests require a current Auth subject. Bypass classification derives only from PostgreSQL's active role or session user and never from JWT claims or metadata. The dashboard runtime is still evaluated because it proxies some already-authorized merchant configuration commands. The isolated worker bypasses only the trigger actor check; worker-driven protected/operational tables are not registered. Migration administration and the database owner retain an explicit subjectless deployment path.

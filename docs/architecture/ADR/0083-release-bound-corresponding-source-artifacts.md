...
- CI source validation installs the exact lockfile with lifecycle scripts disabled before parsing the canonical plan or archive. Build-only archive modules remain dynamically loaded, but the verifier does not pretend its audited parser and streaming dependencies are available on a dependency-free runner.
- Archive verification is streaming and platform-independent. The archive, external manifest, notices, staged source files, and APKBUILD inputs are opened without following the final symlink; identity and bounds are checked on the opened descriptor, the accepted digest is computed from those same bytes, and post-read descriptor/path metadata must remain stable. It rejects duplicate or excess entries, devices, hard links, directories, unexpected modes, size drift, path traversal, and symlink escape before accepting the internal/external manifest and notices as byte-identical.

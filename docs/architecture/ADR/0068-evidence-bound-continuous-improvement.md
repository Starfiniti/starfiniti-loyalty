...
3. Review only canonical official provider, platform, and recovery-dependency sources. Record the installed and candidate version or observed entry, source and package provenance, impact, affected modules, owner, and durable disposition. Automated update tools may collect candidates but cannot approve an upgrade.

...
Run `npm run continuous-improvement:supabase-runtime:validate`, `npm run db:validate`, the focused Auth/session tests, and the complete `npm run check` gate. Exact implementation-head CI must additionally replay every migration and pgTAP file, run concurrency probes and the four WooCommerce runtime cells, rebuild images, and complete security, audit, licence, SBOM, Trivy, CodeQL, and DAST checks before the repository evidence can close. Merge, release, deployment, live stack upgrade, production mutation, and reconciliation remain separate approvals.

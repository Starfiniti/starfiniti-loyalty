...
2. Next.js validates and hashes the token. PostgreSQL stores only SHA-256, exact role, non-authoritative display label, issuer, expiry, state, and acceptance evidence. An exact command retry never re-reveals the token.
...
7. Member changes use optimistic membership revisions. Demoting or revoking an owner requires another active owner. Admins cannot change an owner or promote one.
...
9. Suspension immediately makes M13 permissions ineffective while retaining an owner recovery view. Closure is terminal. Offboarding revokes all non-owner memberships and pending invitations but preserves one owner, immutable audit/ledger evidence, and the minimized administration export. It does not delete loyalty value or rewrite history.
...
Existing organization and membership rows gain additive revisions/timestamps and remain readable through their prior policies. Existing owners remain revision 1. No legacy loyalty command receives a new role or permission. Revocation continues to fail the next database request even if a Supabase access JWT remains valid. The command projection provides labels rather than Auth identity data; labels are presentation only.

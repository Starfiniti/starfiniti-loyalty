...
4. A support read calls one volatile projection function with the public grant ID. PostgreSQL derives the current user and signed session ID, checks the session against `auth.sessions`, rechecks every authority boundary, applies scopes field-by-field, and appends an immutable use event in the same transaction. No support membership or broad RLS path exists.

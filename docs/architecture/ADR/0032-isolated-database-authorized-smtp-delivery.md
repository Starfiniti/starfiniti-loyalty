...
The worker database role receives no direct template, delivery, attempt, event, preference, customer-link, or Auth table access. One Auth-owned security-definer bridge accepts only a database-derived Auth user UUID and returns an email only when that Auth row is confirmed and not deleted. Only the NOLOGIN loyalty function owner can execute the bridge; the worker cannot. The dispatch command has an empty search path, validates worker/lease identity, derives tenant and contact authority internally, and exposes contact only in the authorized result. Templates and attempts are immutable; delivery state is operational projection only. Message rendering disables filesystem and URL access at transport and message level. No SMTP outcome can create, release, reserve, spend, expire, reverse, or reconcile loyalty value.

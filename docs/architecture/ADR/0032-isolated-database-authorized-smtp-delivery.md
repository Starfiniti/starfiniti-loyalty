...
2. PostgreSQL owns a mutable delivery lease projection and append-only template/attempt evidence. Enqueue and migration backfill resolve current deployment/entitlement authority at statement time rather than reusing an event's transaction timestamp; disabling delivery therefore takes effect before any later statement can create work, even inside a long transaction. Direct table access remains revoked. The worker can only claim a bounded batch, authorize its exact owned lease, and finish one allowlisted outcome.

...
3. Bind the published Prometheus and Alertmanager administration ports to loopback only. Grafana and both exporters publish no host port. The central services share an internal control network; only Prometheus, Alertmanager, blackbox_exporter, and postgres_exporter join the separately named egress/scrape network. Prometheus verifies Grafana readiness and its exact version through an internal, build-info-only scrape. Approved HTTPS ingress must be provided by a separately reviewed reverse proxy attached to the control network at the production route gate; the repository candidate does not grant Grafana egress merely to make host NAT work. The approved host firewall remains the authority for exact scrape and receiver destinations.

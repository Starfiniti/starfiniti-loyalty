...
3. Bind Prometheus, Alertmanager, and Grafana administration ports to loopback only. Exporters publish no host port. The central services share an internal control network. Prometheus, Alertmanager, blackbox_exporter, and postgres_exporter use the separately named egress/scrape network as needed. Grafana also attaches to that bridge solely so Docker can implement loopback ingress, but Docker Engine 28.0+ and Compose 2.33.1+ select the internal control bridge as its only default route; dropped `CAP_NET_ADMIN` prevents the process from adding an egress route. The Linux canary matches the kernel default gateway to Docker's internal-network gateway and proves the port bindings. The approved host firewall remains the authority for exact scrape and receiver destinations.

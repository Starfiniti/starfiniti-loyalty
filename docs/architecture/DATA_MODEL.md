...
A valid dry-run receipt is not a value command. M12-S02 must re-present every exact canonical row and mapping, independently revalidate its receipt hashes and database customer/programme relationships, then create source-row-attributed opening-balance ledger transactions and lots. No aggregate receipt count or browser selector can become points authority.

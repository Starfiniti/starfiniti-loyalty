...
- PostgreSQL retains additive tenant current state, immutable revisions, minimized evidence, domain-separated HMAC-SHA256 upstream/broker secret fingerprints under a separately mounted 256-bit key, exact idempotency, one-active-source serialization, public resolver minimization, owner recovery, and zero membership or ledger effects. Legacy V1 `*Sha256` names remain wire-compatible and contain no raw or recoverable credential.
...
- Verified production files must be minimized JSON under the M13 production evidence root, safely opened descriptor-first as bounded regular files, unique by path/digest, candidate-commit and check-coverage exact, semantically valid under [the M13 artifact contract](ARTIFACT_CONTRACT.md), and free of identity, external subject, reusable federation/SCIM/support/recovery material, raw payloads, private selectors, and ledger metadata.
- Every artifact has an exact detail schema. Check-bearing artifacts require unique source digests and zero-difference assertions; the final approval binds every other artifact, and canonical chronology requires prerequisites before canary start, reconciliation/rollback after canary end, at least 24 hours of observation covering the canary, and final approval last.
- Adversarial fixtures reject approval drift, missing/duplicate/forward-looking checks, score drift, short commits, sensitive values, weakened failure rules, hollow semantic evidence, nonzero reconciliation, different approval bindings, impossible chronology, short observation, missing/unsafe/reused/digest-drifted artifacts, unsafe public baselines, incomplete prerequisite slices, prose-only closure, and category-floor bypass; a positive fixture proves valid closure remains attainable.
...
### Separate Authentik 2026.8 candidate-runtime rehearsal

- ADR-0110 adds a fourteen-scenario exact-image runtime rehearsal in the existing Security recovery job. It bundles the production `AuthentikFederationAdmin`, boots an isolated 2026.8.0 server and worker with the exact PostgreSQL dependency, and drives a synthetic bearer-protected SCIM sink without publishing a port, mounting the Docker socket, or providing a production route or credential.
- The rehearsal covers disabled OIDC and SAML reconciliation, idempotent rotation, exact source-only flow bindings, the strict Supabase callback, authorization-code and hashed-subject provider settings, downstream discovery, SCIM service-provider discovery, pagination, default external IDs, group membership, quoted member removal, and deactivation.
- `npm run continuous-improvement:authentik-2026-8:runtime:validate` passes locally. Exact-head Linux execution is pending. Its minimized report is M13/M15/M16 compatibility evidence only: it cannot change the twelve passed or thirty-nine pending M13-S06 checks, and existing pgTAP, two-session, stale-session, RLS, real enterprise IdP, recovery, rollback, observation, and approval gates remain authoritative.

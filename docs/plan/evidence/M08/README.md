...
- Pending authority: the new clean 49-migration replay and 82 focused `notification_smtp_delivery_test.sql` assertions must pass on Linux CI before S02 is marked complete. Those assertions cover grants/RLS, narrow Auth-column authority, event-to-delivery idempotency, ephemeral verified contact, withdrawal, feature rollback, explicit retry/dead-letter/manual-review outcomes, attempt exhaustion, pre/post-authorization crash recovery, unverified contact, template/attempt immutability, and zero ledger change.

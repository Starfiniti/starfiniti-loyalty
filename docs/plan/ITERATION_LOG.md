...
- Added a regression with nine completed V1 policy holds followed by the tenth authorized, expired claim. Recovery must retain claim sequence ten, record provider attempt one as ambiguous, and leave historical policy evidence immutable. The first repair replay reached a previously unexecuted pre-network branch and exposed PL/pgSQL output-column ambiguity in three inferred conflict targets; one named immutable hold-identity constraint now removes that ambiguity. The complete local gate passes 999 tests, all validators, both production builds, 89 static migrations, and 70 static pgTAP files; exact Linux replay remains required before this repair is accepted.

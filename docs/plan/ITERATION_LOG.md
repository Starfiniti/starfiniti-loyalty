...
- The collector requires a clean exact commit, reads the plan through one bounded no-follow descriptor with before/after identity checks, streams at most 4,000,000 bytes per source directly into SHA-256, and records only minimized facts. It rechecks HEAD and plan bytes before creating an absent absolute JSON file through an exclusive no-follow owner-only descriptor and fsync. No provider body is retained.
- Twenty-five deterministic network-free cases reject missing/duplicate/drifted sources, false content/review assertions, invalid digests, private, IPv4-mapped-private, and mixed DNS, insecure, query-bearing, or unapproved redirects, invalid type/encoding, oversized/truncated/incomplete responses, relative or missing-parent output, and overwrite. The first real capture attempt retained no artifact but exposed that the documented `runs/` directory was absent and output-parent validation occurred after collection; the durable directory and a pre-network refusal regression now close that weakness. The focused collector, M16, architecture, lint, formatting, and diff gates pass locally. A real clean-head snapshot, corrected exact-head CI/security, installed endpoint evidence, human impact classification, owners, and elapsed monthly close remain pending; no production source, package, credential, service, schedule, checkout path, or loyalty value changed.

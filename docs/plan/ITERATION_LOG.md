...
- Added a regression with nine completed V1 policy holds followed by the tenth authorized, expired claim. Recovery must retain claim sequence ten, record provider attempt one as ambiguous, and leave historical policy evidence immutable. Exact replay successively exposed a PL/pgSQL output-column ambiguity and same-statement snapshot visibility that leased a new row but could not normalize its just-written token. One named immutable hold-identity constraint and separate procedural claim/normalization statements now fail closed without stranding processing rows. Exact code head `7fb4403863e7409e38bf4fbfc1f3ec68d6325e3f` passed CI `33434852244` and Security `33434852266`: all seven CI jobs, all four Security jobs, 999 workspace tests, both production images, all 89 migrations, all 70 pgTAP files with 3,831 assertions, all 22 two-session concurrency probes, and all four WooCommerce runtime cells are green.
...
## 2026-08-31 — Repeat PostgreSQL off-site lock starvation
...
- Seventeen focused access, stale-command, and real rendered-markup tests pass. The complete local repository gate passes 1,010 workspace tests, both production builds, every deterministic validator, and static validation of 90 migrations and 70 pgTAP files. Exact-head CI/Security checks remain before repository close; provider canary, production, and score state remain unchanged.

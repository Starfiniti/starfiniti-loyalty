...
On 2026-09-01 direct production inspection again proved that VM 971's 3.605 TB
counter is cumulative history rather than an active full-stream loop. The
latest one-day RRD peak was 107 KB/s; the historical month peak of 249.6 MB/s
occurred on 2026-08-14. Host outbound RRD stayed below 279 KB/s during that
window and Borg added only kilobytes per repeated multi-gigabyte input stream,
confirming that the amplification stayed on the internal VM-to-host path. The
later physical `eno1` total includes subsequent whole-host backups. The whole-host controller
was still configured for raw disk mode and its timer remained enabled against
the shared PostgreSQL Borg boundary. Its scheduled run that day failed because
a PostgreSQL cycle held the same lock; a prior raw run had already caused the
exact 1h34m36s off-site database gap. Operations disabled and stopped only
`starfiniti-pve-borg-backup.timer`. Existing whole-host and PostgreSQL archives
were retained, and the enabled PostgreSQL timer created
`loyalty-postgres-20260901T092222Z` after containment. Do not re-enable the
whole-host timer until ADR-0071's dedicated repository is live or an approved
supervised recovery window explicitly accepts the resulting RPO exposure.

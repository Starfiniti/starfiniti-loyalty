...
The ADR-0071 candidate is repository-tested but not deployed. Its automated maintenance requires a fresh archive before taking the dedicated lock, bounds every Borg operation, and fails before a later step on check or timeout failure so maintenance cannot silently recreate the long archive gap. Production continues using the shared repository until an approved operator creates and escrows the dedicated repository, proves the exact repository distinction, validates a retention dry run, installs both reviewed units, completes manual/timer/timeout archive evidence, and restores the dedicated archive in isolation. Existing shared-repository PostgreSQL archives must remain available throughout that transition.

...
3. Review every official provider source in the canonical catalogue. Record the last reviewed release or dated entry, breaking/security/support impact, affected modules, an owner, and the resulting task, ADR, test, upgrade plan, or explicit no-impact rationale.
...
- official provider sources and dependency pins are current through the review cutoff;

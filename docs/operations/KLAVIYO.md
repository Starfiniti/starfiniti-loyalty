...
2. From the exact release source, compute the connection-bound V2 fingerprint without printing or logging the key: `npm run klaviyo:fingerprint -- --connection-id <connection-public-uuid> --key-file <absolute-owner-only-key-file>`. The command uses scrypt with the versioned context, canonical connection UUID, `N=32768`, `r=8`, `p=1`, a 64 MiB memory ceiling, and a 32-byte result. It reads one bounded regular file through an `O_RDONLY|O_NOFOLLOW` descriptor and prints only 64 lowercase hex characters.
3. As a database administrator, create the private connection row for the exact organization with that fingerprint, pinned revision `2026-07-15`, optional list ID, and initial `disabled` state. The legacy database column remains named `credential_sha256` for compatibility, but V2 rows contain the scrypt result rather than raw SHA-256. The key itself never enters PostgreSQL.
...
Rollback never calls subscribe, unsubscribe, profile deletion, or provider suppression removal. Stop only the Klaviyo worker before changing fingerprint versions. A rollback to a V1 worker also requires restoring the disabled connection row's prior SHA-256 value; never accept both fingerprint algorithms concurrently. Re-enable only after verifying the same tenant, connection UUID, key fingerprint, image version, and current entitlement. Safe pre-action event/unsubscribe work can resume after authority is rechecked; ambiguous subscribe remains manual until reconciled.

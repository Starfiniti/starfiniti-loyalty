...
### 5.19 WooCommerce plugin administration

Design a realistic WordPress administration prototype for the Starfiniti Loyalty WooCommerce plugin. The plugin is a thin connector and administration bridge, not a second loyalty platform and not the points ledger.

The boundary must be obvious:

- `loyalty.starfiniti.com` owns programmes, earning rules, rewards, tiers, campaigns, customer balances, the immutable ledger, analytics, referrals, and the full experience builder
- WooCommerce owns orders, products, refunds, channel customers, checkout, and native discount execution
- The plugin connects the systems, renders storefront surfaces, sends signed events, maintains a retry queue, exposes local health, and provides safe recovery tools

Do not duplicate complex programme-management forms in WordPress. Use clear deep links such as `Open Loyalty Dashboard`, `Manage rewards`, and `View full analytics` to hand off to the correct SaaS workspace.

#### WordPress navigation and visual language

- Place the product under `WooCommerce > Loyalty`; do not add a separate top-level WordPress menu
- Use a React-powered WooCommerce Admin page registered inside the WooCommerce shell, with native WordPress and WooCommerce components
- Do not embed the SaaS dashboard in an iframe
- Keep critical connection status, reconnect, safe disconnect, and recovery actions available through a server-rendered fallback if the JavaScript application fails
- If the implementation adopts WooCommerce's newer React Settings UI, retain the established `WC_Settings_Page` schema and legacy renderer fallback while that interface remains opt-in
- Match the current WooCommerce admin hierarchy, spacing, notices, tables, buttons, tabs, focus states, and permission behavior
- Use the Starfiniti mark sparingly in the page header and connection card; do not make wp-admin look like a different application
- Keep the primary navigation compact: `Overview`, `Storefront`, `Sync`, `Diagnostics`, and `Settings`
- Make the page work at normal wp-admin desktop widths and narrower tablet widths

Prototype routes:

- `/demo/woocommerce-admin/loyalty`
- `/demo/woocommerce-admin/loyalty/connect`
- `/demo/woocommerce-admin/loyalty/storefront`
- `/demo/woocommerce-admin/loyalty/sync`
- `/demo/woocommerce-admin/loyalty/diagnostics`
- `/demo/woocommerce-admin/loyalty/settings`

Render the recognizable WordPress admin frame in these demonstration routes, including the left navigation, WooCommerce header, notices, content width, and responsive collapsed navigation. Label it clearly as a prototype rather than copying a live merchant site.

#### First-run connection flow

Design a short guided setup:

1. Welcome and value statement
2. Environment checks for WooCommerce version, HTTPS, permalinks, REST API, cron or Action Scheduler, and HPOS status
3. `Connect to Starfiniti Loyalty` using a secure one-time authorization flow
4. Select or create the SaaS organization, workspace, and store connection
5. Review requested scopes and data events in plain language
6. Run an initial customer and order reconciliation with progress, background-safe navigation, and retry behavior
7. Enable recommended storefront surfaces and show placement instructions
8. Test the connection and open the customer-facing preview

Support these setup states: new, authorization pending, connected, partially synchronized, permission missing, expired authorization, reconnect required, and connection failed. Preserve progress and explain recovery without asking the merchant to reinstall the plugin.

#### Overview tab

The overview should answer three questions immediately: Is loyalty working? Is data current? What needs attention?

Include:

- Header with store name, environment badge, connection status, and primary `Open Loyalty Dashboard` action
- Read-only summary cards for members, points issued in 30 days, rewards redeemed in 30 days, and member revenue, each linking to the SaaS analytics view
- Health strip for API connection, webhooks, background queue, storefront components, and plugin compatibility
- Last successful synchronization and last processed WooCommerce order
- Action checklist for unfinished setup or degraded states
- Recent event list using human-readable order and customer context
- Compact notices for errors, warnings, plugin updates, and planned maintenance

Do not turn this into a second analytics dashboard. Four read-only metrics are enough.

#### Storefront tab

Show the install and visibility state of every customer-facing component:

- Rewards launcher and wallet drawer
- Product-page points message
- Cart reward module
- `My Account > Rewards`
- Checkout message where supported
- Thank-you and order-status summary
- Dedicated rewards page

For each surface show status, detected placement, audience, last render check, and actions for `Preview`, `Test`, `Enable or disable`, and `Placement help`. Use safe defaults and progressive disclosure.

Include:

- Block-theme and classic-theme guidance
- Detection when a required block has not been placed
- Guest and member preview states
- Desktop and mobile preview links
- A global emergency switch to hide loyalty storefront components without disconnecting event processing
- `Edit design in Starfiniti` and `Edit programme copy` links to the SaaS experience builder
- An advanced placement section for supported hooks and selectors, collapsed by default

#### Sync tab

Design the sync area for operators, not developers:

- Initial-sync or reconciliation progress with totals for customers, orders, refunds, and coupons
- Last received and last acknowledged event
- Queue summary: pending, processing, retrying, failed, and dead-lettered
- Filterable job table with event type, WooCommerce object, attempts, next retry, and status
- Human-readable error explanation with `Retry`, `Inspect`, and `Resolve` actions
- Date range and object-type controls for targeted backfill
- Safe `Run reconciliation` and `Sync now` actions with confirmation and progress
- Clear distinction between data waiting locally, accepted by Starfiniti, rejected, and already reconciled

The browser must not need to remain open for a long-running sync. Show that work continues through Action Scheduler.

#### Diagnostics tab

Include one-click health checks for:

- Starfiniti API reachability and signed request validation
- Webhook endpoint and recent delivery status
- REST API and permalink configuration
- Action Scheduler or WP-Cron operation
- HPOS compatibility and active order storage
- WooCommerce, WordPress, PHP, and plugin versions
- Cache, CDN, security plugin, and content-security-policy conflicts
- Required database tables and write permissions
- Storefront script load and component render checks

Also include:

- Redacted logs with severity, time, request or event ID, and copyable context
- Log-retention selector
- `Download support bundle` with secrets and personal data removed
- `Copy system status`
- `Test connection`, `Send test event`, and `Recheck storefront`
- A visible statement that diagnostics never expose API secrets

#### Settings tab

Group only essential local settings:

- Connection: workspace, store identity, environment, reconnect, and disconnect
- Event delivery: enabled event families and retry behavior, with safe recommended defaults
- Privacy: customer identity exchange, consent behavior, retention, and data-erasure handling
- Storefront fallback: behavior during service outage and local cache duration
- Permissions: which WordPress roles may view status, manage placement, run recovery actions, and disconnect
- Logging: severity and retention
- Uninstall behavior: preserve connection data, remove local plugin data, or request SaaS disconnection

Put destructive actions in a separate danger zone with typed confirmation. Never show raw client secrets after connection. Allow credential rotation without downtime.

#### Required plugin states and design details

Prototype connected healthy, attention required, degraded, disconnected, syncing, empty, permission denied, and service unavailable states. Also show:

- Success, warning, error, and informational notices
- Skeleton loading instead of layout jumps
- Empty-state explanation plus one useful action
- Accessible tables, filters, pagination, tooltips, keyboard behavior, and screen-reader labels
- Capability-aware controls that explain why an action is unavailable
- Translation-ready copy that tolerates long labels
- No fabricated green status: health badges must be based on explicit test results and show the time checked
...
19. Connect the simulated WooCommerce plugin to a Starfiniti workspace.
20. Enable and test a storefront component from the WooCommerce plugin dashboard.
21. Inspect a failed synchronization job, retry it, and see the health state recover.
22. Run diagnostics and download a redacted demonstration support bundle.
...
15. WooCommerce plugin administration prototype with setup, overview, storefront, sync, diagnostics, and settings
16. Healthy, degraded, disconnected, syncing, permission, and recovery states for the WooCommerce plugin
...
- Connect the WooCommerce plugin to the correct Starfiniti workspace
- Understand connection, synchronization, queue, and storefront health
- Recover from a failed event without reinstalling the plugin
- Open the hosted dashboard for programme management without losing store context

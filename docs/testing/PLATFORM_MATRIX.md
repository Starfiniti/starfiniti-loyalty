...
The minimum/current smoke is a Phase 7 pass, not the exhaustive production release matrix. R-008 remains open until every declared PHP minor and the remaining money, tax, cache, queue, reward, lifecycle, multilingual, and multi-currency cases have retained evidence. Docker remains unavailable locally, so GitHub Actions is the container-backed runner.

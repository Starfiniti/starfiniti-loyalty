...
- Minimized outbound notification payloads remain Confidential because opaque customer IDs and loyalty activity are linkable; they may leave the platform only after current purpose authorization.
...
- Outbound webhook HMAC keys are Restricted mounted-file secrets. Database rows store only SHA-256 fingerprints; attempts never retain payloads, signatures, contacts, response bodies, or destination credentials.

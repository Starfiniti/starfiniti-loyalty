...
The server reserves the operation in `loyalty_private.reserve_managed_billing_session_v2` before loading an API key or constructing a provider client. PostgreSQL returns before billing inspection in self-hosted mode, then checks the active organization, live owner membership, `managed.billing` entitlement, provider mode, and effective plan/account evidence. Checkout is serialized per organization and fails closed when normalized evidence already contains a live subscription or another unresolved Checkout operation. An exact retry returns the same fence; changed content under the same operation ID fails with SQLSTATE `23505`.

Immediately before each customer or session request, `authorize_managed_billing_session_attempt_v2` rechecks membership, entitlement, provider mode, and—under the same organization lock—live-subscription absence for Checkout. The narrow client can POST only to Stripe's fixed API origin, pins reviewed API version `2026-02-25.clover`, uses a stable database-derived idempotency key, pins success/cancel/return navigation to `DASHBOARD_PUBLIC_ORIGIN`, limits responses to 32 KiB, and accepts redirects only from Stripe Checkout or Billing Portal origins. Customer creation sends only the opaque operation ID and no email.

Provider retry authority expires at the stage-specific 23-hour deadline, before Stripe may prune an idempotency result. An unresolved request at or after that deadline becomes `reconciliation_required` and cannot be sent again. Only an owner-role private operation can append minimized immutable reconciliation evidence; exact decisions converge and changed decisions fail closed. Database constraints independently reject a second non-cancelled provider subscription identity for an organization. The managed Stripe account must also enable its one-subscription control before canary activation.
...
`record_managed_billing_usage_meter_v1` stores externally supplied append-only event-name configuration only in managed mode. It contains no price. `capture_managed_billing_usage_facts_v2` returns before scanning product sources in self-hosted mode and resolves tenant billing eligibility at each immutable source occurrence time, so later activation cannot backfill earlier activity. It recognizes:

- the first canonical event for one tenant, commerce connection, and order object, using that event's canonical occurrence time rather than ingestion time;
- one customer with an immutable loyalty ledger transaction per UTC month;
- one delivered SMTP message; and
- one accepted customer command receipt or signed Service API activity event.

Klaviyo HTTP acceptance and completed synchronization are not delivery evidence and therefore do not create message usage. They can become billable only through a future versioned provider-neutral delivered-event contract.

The isolated billing worker claims only a public dispatch selector and lease through `claim_managed_billing_usage_dispatches_v2`. `authorize_managed_billing_usage_dispatch_v2` then rechecks deployment mode, `managed.billing`, current account, provider mode, current meter version, provider timestamp window, worker, and lease before returning one minimized provider payload. Only after that row exists does the worker read its separate regular-file restricted key and construct the fixed-origin Stripe client. `begin_managed_billing_usage_provider_attempt_v1` increments the ten-attempt provider budget immediately before the network send. Claim sequences, lease expiry, missing local configuration, and other pre-network policy holds do not consume that budget. During a V1-to-V2 worker upgrade, completed V1 rows rebuild the two counters; an expired authorized V1 claim retains its legacy claim identity and becomes exactly one ambiguous provider attempt, while an expired claim before authorization becomes a policy hold with zero provider attempts.

Each fact has one permanent `m14u_…` identifier used as both the Stripe meter-event identifier and HTTP idempotency key. Exact duplicate responses are accepted. Temporary HTTP failures receive bounded retries; policy holds cool for five minutes; a timeout or interrupted response is retained as ambiguous for reconciliation instead of being replayed after Stripe's bounded duplicate-enforcement window. A crash after provider-attempt authorization is conservatively ambiguous. Signed negative quantities are used only for append-only compensation, which Stripe documents for correcting usage.

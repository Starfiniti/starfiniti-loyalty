...
6. Copy `.env.example` to a secret-managed location outside Git and replace every placeholder. From the matching release source run `npm run deploy:preflight -- --env /absolute/path/to/starfiniti.env`; it validates Compose parity, immutable image selectors, HTTPS origins, distinct least-privilege database logins, signing-pool structure, and Linux owner-only permissions without printing values.

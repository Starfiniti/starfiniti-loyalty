...
M01-S01 remains the active infrastructure compatibility slice under ADR-0081 and ADR-0123. Its fail-closed contract now covers 90 migrations, all 175 tenant tables, exact upstream source provenance, both approved Compose byte variants, fifteen mounted assets, eleven reviewed `linux/amd64` image IDs, and required gateway/Auth/Data API/database behavior. On 2026-09-01 the owner-approved dry run and isolated no-pull/no-build/no-dependency recreation changed only the stale Studio container. Studio now carries the exact `public,graphql_public,loyalty` runtime list on the same reviewed healthy image and its internal profile endpoint returns HTTP 200 while the public route remains HTTP 404; the other ten services remained healthy, a post-change PostgreSQL archive passed, public dashboard/login passed, anonymous Auth/Data API access remained denied, and all protected commerce/value aggregates remained zero. The immutable 2026-08-28 baseline is hash-bound by a V2 successor and 52 corruption cases. A future exact upgrade rehearsal and clean-room restore remain the compatibility closeout gates; no database, application, Auth, Data API, backup configuration, checkout, or loyalty value changed.
...
- The self-hosted Supabase source bundle has an exact offline compatibility contract and 52-corruption self-test. Repository/static checks cover 90 migrations, all 175 tenant tables, two Compose variants, fifteen mounted assets, and eleven platform image IDs. The bounded production Studio recreation now has exact runtime schema parity plus internal HTTP 200/public HTTP 404 profile smokes, post-change backup, service, anonymous-denial, and protected-state evidence; exact future upgrade rehearsal and full clean-room recovery remain explicitly pending.
...
- M01 Supabase compatibility closeout: approved exact future upgrade rehearsal and clean-room recovery exercise; the isolated Studio recreation/smoke is complete.

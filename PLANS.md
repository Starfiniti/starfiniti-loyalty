...
M09-S01 through S04 are complete on draft PR #36. The Auth-derived hosted experience covers overview, earning, rewards, VIP, referrals, immutable history, and account states with desktop/mobile/reflow evidence. ADR-0037 added demand-driven signed WooCommerce snapshots and classic placements; ADR-0038 added official namespaced Store API data and a separately flagged Blocks panel using only strict local state. Exact-head run `32859649418` at `bf5ec90` passed baseline, both images, a clean 56-migration replay, all 44 pgTAP files with 2,392 assertions, and all four WooCommerce runtimes with staged flags, real Store API payloads, native coupons, no-script fallback, and forced Hub failure. Desktop/mobile browser review passed the exact asset budgets and interaction/failure checks. M09-S05 controlled branding, accessibility, responsive/offline presentation, and English-only hardening is active.

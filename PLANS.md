...
M09-S01 through S03 are complete on draft PR #36. The Auth-derived hosted experience covers overview, earning, rewards, VIP, referrals, immutable history, and account states with desktop/mobile/reflow evidence. ADR-0037 then added demand-driven signed WooCommerce snapshots and My Account, product, cart, classic checkout, and post-purchase placements using only strict local non-autoloaded state. Exact-head run `32853757058` at `c2e2c82` passed baseline, both images, a clean 56-migration replay, all 44 pgTAP files with 2,392 assertions, and all four WooCommerce runtimes under forced Hub failure. M09-S04 now adds official namespaced Store API data and a separately flagged, budgeted Blocks panel while preserving the complete no-script path.
...
1. M09: complete S04 namespaced Store API data, official Cart/Checkout Blocks SlotFill integration, a separately disabled progressive panel, no-script fallback, and compressed asset budgets.

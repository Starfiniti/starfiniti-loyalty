...
M09-S01 and S02 are complete on draft PR #36. ADR-0036, strict `CustomerLoyaltyExperienceV1`, one Auth-derived PostgreSQL statement snapshot, minimized earning summaries, enhancement-only entitlement behavior, and fail-closed server parsing passed run `32839387263`. The resulting feature-flagged Hub-style member experience covers overview, earning, rewards, VIP, referrals, immutable history, and account states; the bounded guest experience routes to a same-origin English account path. Desktop, mobile, and 200%-reflow real-component browser review passed, and exact-head run `32844158775` at `ad1ddcf` passed baseline, both images, a clean 55-migration replay, all 43 pgTAP files with 2,361 assertions, and all four WooCommerce runtimes. M09-S03 now builds the signed local WooCommerce snapshot and classic placements without render-time Hub dependency.
...
1. M09: complete S03 signed local WooCommerce snapshot delivery and My Account, product, cart, checkout, and post-purchase classic placements with zero render-time Hub dependency.

...
M05-S01 and S02 are exact-head green: strict policy contracts, event-time qualification, immutable facts, original-time refund compensation, worker integration, and independently verified automatic membership history passed clean replay and the complete runtime matrix. M05-S03 locally adds executable tier multipliers, fulfilment-bound benefit rewards, expiring reason-bound owner/admin overrides, continued underlying automatic qualification, and exactly-once expiry; exact-head database CI is the next gate.
...
1. M05: finish exact-head benefits/override verification, then deliver expiry policy, progression experiences, shadow comparison, disabled deployment, canary reconciliation, and score.

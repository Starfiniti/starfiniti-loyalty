...
- The WooCommerce localization boundary registers the packaged `/languages` path at `init` and maintains exact POT coverage. The product-owner-approved launch is now English-only; no bundled locale catalog or locale-bearing claim link is shipped.
...
- Hosted customer copy presents and edits only the bounded English revision. Existing locale-keyed storage remains for migration compatibility, while unsupported or legacy selectors resolve to English and are removed from navigation.
...
- Hosted login, WooCommerce claim, member-account, and reward-redemption routes are English-only. Safe navigation removes legacy locale parameters, and WooCommerce claim links are language-neutral.
...
- The authenticated Overview, programme lifecycle, connector operations, customer administration, and experience editor now use one canonical English presentation with no language switcher. Legacy `lang` selectors are ignored and stripped from authenticated and guest navigation. Europe/Ljubljana administrative wall-time parsing and DST rejection remain unchanged.
...
- PR `#20` exact-head run `31713151458` passed all seven jobs, and release run `31713458344` published and deployed `v0.1.10` at immutable commit `4713c65e4ca47c0a97264854afea46f6a8730a3a`. Authenticated desktop/mobile production QA passed all four Programme workflows with zero browser diagnostics.
...
- `P9-MERCHANT-HUB` (in progress): repository implementation, release hardening, Authentik-to-Supabase workforce SSO, English-only presentation, verified owner tenant, the approved Hub-style shell, and focused Programme workflows are live on Proxmox as `v0.1.10`. The remaining launch work is a real WooCommerce connection and authenticated/outage-recovery smoke tests.

## Next safe tasks

1. Connect the real WooCommerce store and execute authenticated connector/outage-recovery smoke tests.
2. Complete the full application/Auth/secret-escrow recovery smoke; the encrypted off-host database/WAL replay rehearsal already passes.

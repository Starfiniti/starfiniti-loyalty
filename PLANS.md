...
M07-S01 through S05 are exact-head green on draft PR #32. Strict canonical-fact audiences, immutable database-timed snapshots, seven closed campaign behaviors, explicit-instant/IANA schedules, exact budgets, private approval-bound control assignment, atomic value execution, canonical trigger queues, campaign-funded native rewards, and exactly-once refunds/reversals now have a complete Hub-style merchant command center and minimized exact results projection. Run `32671197966` passed the complete repository gate, both images, a clean 46-migration/38-pgTAP replay with 1,975 assertions including 123 focused campaign assertions, all five concurrency probes, 139 dashboard tests, 25 worker tests, 170 contract tests, 57 domain tests, 19 validator-tracked ADRs, and all four WooCommerce runtimes at `fcb1f3c`. Production-build desktop/mobile browser review passed keyboard, navigation, dynamic builders, dark mode, overflow, and diagnostic checks. M07-S06 reviewed stacked merge, disabled deployment, Starfiniti-only canary, full reconciliation, and scoring is active; no production campaign schedule, entitlement canary, or value is live yet.

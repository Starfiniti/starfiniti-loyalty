...
M07-S01 and S02 are exact-head green on draft PR #32. Strict canonical-fact audiences, immutable database-timed snapshots, seven closed campaign behaviors, explicit-instant/IANA schedules, exact budgets and decimal RPC values, private approval-bound salted-hash control assignment, and Auth-derived draft/preview/approve/pause/cancel lifecycles passed runs `31773939480` and `31777177961`. The latter passed the complete repository gate, both images, a clean 43-migration/37-pgTAP replay with 1,851 assertions including 81 focused campaign assertions, all four concurrency probes, 126 dashboard tests, 21 worker tests, 157 contract tests, 53 domain tests, 16 ADRs, and all four WooCommerce runtimes. Adversarial review found and resolved every blocker and should-fix issue. M07-S03 atomic capacity and execution is active; no campaign schedule or value is live yet.
...
1. M07: implement atomic campaign capacity reservation and exactly-once execution in S03 on the exact-green S01/S02 authority.

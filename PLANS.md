...
M08-S02 is implemented at repository checkpoint `1fce7a6` and remains under exact-head Linux verification. ADR-0032 isolates an optional SMTP worker from value processing, uses database-owned bounded leases and dispatch-time consent/entitlement/contact authorization, pins six immutable English templates, and stops ambiguous remote acceptance in manual review. Local typechecks, worker build, 45 worker tests including a real loopback SMTP sink, 20 focused contract tests, lint, migration/deployment/workflow validators, and secret/diff scans pass. The clean 49-migration replay and 86 focused SMTP pgTAP assertions are pending before S02 can close. No production SMTP credentials or delivery are active.

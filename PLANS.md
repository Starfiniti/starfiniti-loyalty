...
M06-S01 is active on draft PR #31. The strict policy, opaque Auth-linked advocate identity, offline WooCommerce capture, purpose-separated expiring risk fingerprints, database-derived first attribution, deterministic self-referral block, reversible review state, and rollout rollback are implemented. Local lint, all workspace typechecks, 305 tests, static migration validation, PHP syntax, architecture, and WooCommerce validators pass; exact clean database replay and the four-runtime matrix remain pending CI run `31762510623`.
...
1. M06: close exact CI for first attribution, then implement qualification/cooling, two-sided ledger value, reversible review, and customer/merchant experiences.

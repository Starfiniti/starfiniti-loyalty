...
M05-S01 through S03 are exact-head green: strict policy contracts, event-time qualification, immutable facts, original-time refund compensation, executable tier multipliers, fulfilment-bound benefit rewards, expiring reason-bound owner/admin overrides, continued underlying automatic qualification, and exactly-once override expiry passed clean replay and the complete runtime matrix. M05-S04 earned-date policy, version-scoped expiry automation, reminder scheduling, original-lot restoration, and aggregate liability preview are implemented with local unit/static gates green; clean database replay and browser evidence remain pending before the slice can close.
...
1. M05: finish exact-head and browser proof for expiry policy/lifecycle automation, then deliver progression experiences, shadow comparison, disabled deployment, canary reconciliation, and score.

...
import {
  ...
  type PublicLoyaltyExperienceV6,
...
export async function getPublicLoyaltyExperience(
  workspaceId: string,
  programmeId: string,
): Promise<PublicLoyaltyExperienceV6 | null> {
  const loyalty = createPublicSupabaseClient().schema("loyalty");
  const v6 = await loyalty.rpc("get_public_loyalty_experience_v6", {
    target_workspace_public_id: workspaceId,
    target_programme_public_id: programmeId,
  });
  if (!v6.error) return parseV6(v6.data);
  if (!isMissingProjection(v6.error)) {
  ...
  return parsed
    ? normalizeV5(normalizeV4(normalizeV3(normalizeV2(normalizeV1(parsed)))))
    : null;
}

function parseV6(data: unknown): PublicLoyaltyExperienceV6 | null {
  ...
  const row = data[0];
  const parsed = publicLoyaltyExperienceV6.safeParse({
    version: "6",

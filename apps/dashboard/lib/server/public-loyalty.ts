...
export async function getPublicLoyaltyExperience(
  workspaceId: string,
  programmeId: string,
): Promise<PublicLoyaltyExperienceV3 | null> {
  ...
  if (!v3.error) return parseV3(v3.data);
  ...
  if (!v2.error) {
    ...
    return parsed ? normalizeV2(parsed) : null;
  }
  ...
  return parsed ? normalizeV2(normalizeV1(parsed)) : null;
}
